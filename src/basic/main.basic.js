...
function updateProductSelectOptions() {
  ...
  productList.forEach(item => {
    ...
    if (item.stock === 0) {
      optionElement.textContent = `${item.name} - ${item.price}원 (품절)${discountText}`;
      ...
      if (item.onSale && item.suggestSale) {
        ...
        optionElement.textContent = `${item.name} - ${item.price}원${discountText}`;
      }
    }
...
function createCartItemElement(item) {
  ...
  const priceHtml = item.onSale || item.suggestSale
    ? `<span class="line-through text-gray-400">₩${item.originalPrice.toLocaleString()}</span> <span class="${
...
function calculateCartAndupdateUI() {
  ...
  totalAmount = cartItems.reduce((acc, cartItem) => {
    ...
    let discountRate = 0;
    if (quantity >= 10) {
      if (currentItem.id === PRODUCT_ID_KEYBOARD) discountRate = 0.1;
      else if (currentItem.id === PRODUCT_ID_MOUSE) discountRate = 0.15;
      else if (currentItem.id === PRODUCT_ID_MONITOR_ARM) discountRate = 0.2;
      else if (currentItem.id === PRODUCT_ID_SPEAKER) discountRate = 0.25;
...
function triggerLightningSale() {
  ...
  if (luckyItem.stock > 0 && !luckyItem.onSale) {
    luckyItem.price = Math.round(luckyItem.originalPrice * 0.8);
