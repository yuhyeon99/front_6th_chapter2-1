    ...
    if (product.onSale && product.suggestSale) {
      return (
        <>
          ...
          <span className="text-purple-600"> ₩{product.val.toLocaleString()}</span>
        </>
      );
      ...
      return (
        <>
          <span className="line-through text-gray-400">₩{product.originalVal.toLocaleString()}</span>
          <span className="text-blue-500"> ₩{product.val.toLocaleString()}</span>
        </>
      );
    } else {
    ...
    <div id={item.id} className="grid grid-cols-[80px_1fr_auto] gap-5 py-5 border-b border-gray-100 first:pt-0 last:border-b-0 last:pb-0">
      ...
      <div>
        ...
        <div className="flex items-center gap-4">
          <button className="quantity-change w-6 h-6 border border-black bg-white text-sm flex items-center justify-center transition-all hover:bg-black hover:text-white" data-product-id={product.id} data-change="-1">−</button>
